            ...
            {messages.map((msg, idx) => {
                ...
                return (
                    <div key={idx} className={`flex ${isMe ? 'justify-end' : 'justify-start'} group`}>
                        <div className={`max-w-xs md:max-w-md p-4 rounded-2xl shadow-lg transition-all duration-200 hover:scale-[1.01] ${isMe ? 'bg-gradient-to-br from-blue-600 to-purple-600 text-white rounded-br-sm' : 'bg-white/10 backdrop-blur-md text-gray-200 border border-white/5 rounded-bl-sm'}`}>
                            ...
                            {msg.attachments?.map((att, i) => (
                                <div key={i} className="mb-2 rounded-lg overflow-hidden border border-white/10">
                                    <img src={att.url.startsWith('http') ? att.url : `${import.meta.env.VITE_BACKEND_URL || 'http://localhost:5001'}${att.url}`} alt="attachment" className="max-w-full max-h-60 object-cover" />
                                </div>
                            ))}
                            <p className="break-words leading-relaxed text-[15px]">{decryptMessage(msg.text)}</p>
                            <div className={`flex items-center justify-end space-x-1 mt-1.5 opacity-70 group-hover:opacity-100 transition-opacity`}>
                                ...
                                {isMe && <span className="text-[10px] text-blue-200">{msg.status === 'read' ? '✓✓' : '✓'}</span>}
                            </div>
                        </div>
                    </div>
                );
                ...
                <div className="flex space-x-3 mb-3 overflow-x-auto p-2">
                    {attachments.map((att, i) => (
                        <div key={i} className="relative group">
                            <img src={att.url.startsWith('http') ? att.url : `${import.meta.env.VITE_BACKEND_URL || 'http://localhost:5001'}${att.url}`} alt="preview" className="w-20 h-20 object-cover rounded-lg border border-white/20 shadow-lg" />
                            <button onClick={() => setAttachments(attachments.filter((_, idx) => idx !== i))} className="absolute -top-2 -right-2 bg-red-500 hover:bg-red-600 text-white rounded-full p-1 shadow-md transition-colors"><X size={12} /></button>
                        </div>
                    ))}
                </div>
            )}
            <form onSubmit={handleSend} className="flex space-x-3 items-end relative z-20">
                {showEmoji && (
                    <div className="absolute bottom-20 left-0 z-50 shadow-2xl rounded-2xl overflow-hidden border border-white/10">
                        <EmojiPicker theme="dark" onEmojiClick={(emojiObject) => setNewMessage(prev => prev + emojiObject.emoji)} />
                    </div>
                )}
                <input type="file" multiple ref={fileInputRef} className="hidden" onChange={handleFileSelect} />
                <div className="flex space-x-1 bg-black/20 rounded-xl p-1 border border-white/5">
                    <button type="button" onClick={() => setShowEmoji(!showEmoji)} className="p-2.5 text-gray-400 hover:text-yellow-400 transition-colors rounded-lg hover:bg-white/5"><Smile size={20} /></button>
                    <button type="button" onClick={() => fileInputRef.current?.click()} className="p-2.5 text-gray-400 hover:text-blue-400 transition-colors rounded-lg hover:bg-white/5"><Paperclip size={20} /></button>
                </div>
                <input className="flex-1 glass-input rounded-xl px-4 py-3 focus:ring-2 focus:ring-blue-500/50" placeholder="Type a message..." value={newMessage} onChange={handleTyping} />
                <button type="submit" className="p-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-xl shadow-lg hover:shadow-blue-500/25 active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed" disabled={!newMessage.trim() && attachments.length === 0}><Send size={20} /></button>
            </form>
        </div>
    </div>
);
