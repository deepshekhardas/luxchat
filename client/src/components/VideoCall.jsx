    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-sm">
      <div className="relative w-full max-w-4xl bg-[#1f1f2e] rounded-2xl overflow-hidden shadow-2xl border border-white/10 m-4">
        ...
        <div className="absolute bottom-10 left-1/2 transform -translate-x-1/2 flex items-center space-x-6">
          <button
            onClick={toggleMic}
            className={`p-4 rounded-full ${micOn ? 'bg-white/20 hover:bg-white/30' : 'bg-red-500 hover:bg-red-600'} text-white transition-all`}
          >
          ...
          <button
            onClick={endCall}
            className="p-4 rounded-full bg-red-500 hover:bg-red-600 text-white transition-all scale-110 shadow-lg shadow-red-500/50"
          >
            <X size={32} />
          </button>
          ...
          <button
            onClick={toggleVideo}
            className={`p-4 rounded-full ${videoOn ? 'bg-white/20 hover:bg-white/30' : 'bg-red-500 hover:bg-red-600'} text-white transition-all`}
          >
          ...
          {isReceivingCall && !callAccepted && (
            <button
              onClick={answerCall}
              className="p-4 rounded-full bg-green-500 hover:bg-green-600 text-white transition-all scale-110 animate-bounce shadow-lg shadow-green-500/50 ml-4"
            >
              Answer
            </button>
          )}
