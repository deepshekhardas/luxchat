 ...
 * @access  Private
 */
router.post('/chat', protect, (req, res) => __awaiter(this, void 0, void 0, function* () {
    try {
        const { message } = req.body;
        if (!message) {
            return res.status(400).json({ message: 'Message is required' });
        }
        const response = yield getGeminiResponse(req.user._id.toString(), message);
        res.json({
            success: true,
            data: {
                message: response,
 ...
 * @access  Private
 */
router.delete('/chat/history', protect, (req, res) => __awaiter(this, void 0, void 0, function* () {
    try {
 ...
 * @access  Private
 */
router.post('/sentiment', protect, (req, res) => __awaiter(this, void 0, void 0, function* () {
    try {
        const { message } = req.body;
        if (!message) {
            return res.status(400).json({ message: 'Message is required' });
        }
        const sentiment = yield analyzeSentiment(message);
        res.json({
            success: true,
            data: sentiment
        });
 ...
 * @access  Private
 */
router.post('/smart-replies', protect, (req, res) => __awaiter(this, void 0, void 0, function* () {
    try {
        const { message, context } = req.body;
        if (!message) {
            return res.status(400).json({ message: 'Message is required' });
        }
        const replies = yield getSmartReplies(message, context || '');
        res.json({
            success: true,
            data: {
                suggestions: replies
